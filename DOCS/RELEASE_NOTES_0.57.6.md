...
**What the PR ships.** Executing the player no longer calls `SaveSystem.DeleteSave` or the matching multi-table online `DELETE` — instead it calls `Player.Die()`, routing the character through the normal death/resurrection flow. Prison sentence reduced 3 days → 2 days. Prison punishment softened: half of on-hand gold taken, equipment and inventory and bank gold kept (was: everything stripped, including BankGold). New mid-sentence "surrender or fight" branch — before the 50/50 execution/prison roll, the player can elect to fight 5 scaling "Royal Guards." Win and you escape capture entirely (at +100 darkness). Lose but survive and you're captured. Die and you go through normal resurrection. `MurderDarknessGain` constant bumped 50 → 250, paired with a matching -125 chivalry.

**Cleanup on top of the merge.**

- **Fixed the "kill all guards still went to prison" bug.** The PR checked `result.Victory` after `CombatEngine.PlayerVsMonsters`, but that flag is only populated by berserker-mode combat — the normal victory path only sets `result.Outcome = CombatOutcome.Victory`. So after killing all 5 guards, `result.Victory` stayed `false` and the code fell through to the "arrested" branch. Player report: "I killed all the guards, but still got sent to prison. I should escape with just a darkness penalty." Now checks `result.Outcome == CombatOutcome.Victory`, which is the canonical signal. Renamed the monster literal from "Murder Guard" → "Royal Guard" while I was in there (the PR's name was unintentionally funny).
...
- Reverted `StreetEncounterSystem.CreateRandomHostileNPC` visibility `internal` → `private`. The PR bumped it but nothing in the PR calls it — the 5 Royal Guards are constructed as raw `new Monster { ... }` literals. Leftover from an earlier approach.
