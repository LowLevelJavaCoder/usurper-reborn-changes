...
public class MainStreetLocation : BaseLocation
{
    ...
    protected override async Task<bool> ProcessChoice(string choice)
    {
        ...
        switch (upperChoice)
        {
            ...
            // Save Files, Force Daily Reset, Change Daily Cycle Mode. In MUD /
            // online mode any non-admin user could delete the admin's save file
            // through this path. The visible [~] hotkey routes to the safe
            // per-player ShowPreferencesMenu in BaseLocation (combat speed,
            // auto-heal, intimate scenes, etc.) which is the only thing players
            // ever needed from "settings". ShowSettingsMenu now also has a
            // defense-in-depth guard so even direct callers can't reach the
            // save-management UI in any door / online / MUD mode.
